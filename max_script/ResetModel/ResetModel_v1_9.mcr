macroScript ResetModel_v1_9
category:"ZinAllTools"
tooltip:"ResetModel v1.9: Group Unlock, Exact Naming, Full Hierarchy"
(
    rollout ResetModel_UI "ResetModel v1.9" width:300 height:175
    (
        -- 介面需求
        ...
        (
            ...
            (
                ...
                (
                    ...
                    (
                        -- 放棄 uniqueName 防衝突，強制同名以符合 BOM 料號
                    ...
                    writeLog ("Cleaned Geometry: " + node.name)
                )
                else
                (
            ...
            (
                ...
                -- 3. 矩陣核彈：徹底洗白這個 Helper 的 Rotation 與 Scale
                node.transform = matrix3 1

                -- 4. 恢復原本的位置
                node.pos = currentPos

                -- 5. 重新連結：將 children 接回這個洗白後的 Helper
                for c in myKids do c.parent = node
                
                writeLog ("Neutralized Transform for Helper: " + node.name)
            )
        )
        ...
        (
            ...
            -- 初始化 Log 並寫入開頭
            initLogFile()
            writeLog "=== ResetModel Auto Clean Started (v1.9) ==="
