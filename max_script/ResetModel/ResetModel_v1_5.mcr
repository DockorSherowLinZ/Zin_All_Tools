macroScript ResetModel_v1_5
category:"ZinAllTools"
tooltip:"ResetModel v1.5: Sync Pivot & Rotation, Clean, Optional Marker"
(
    rollout ResetModel_UI "ResetModel v1.5" width:300 height:145
    (
        -- 介面需求
        ...
        (
            ...
            (
                ...
                (
                    ...
                    -- Step B [校正軸心與同步父層] (底部中心)
                    local bMin = node.min
                    local bMax = node.max
                    node.pivot = [(bMin.x + bMax.x) / 2.0, (bMin.y + bMax.y) / 2.0, bMin.z]
                    
                    -- 同步父層軸心 (位置)
                    if originalParent != undefined do originalParent.pivot = node.pivot
                    ...
                    (
                        ...
                        -- 核心對齊: 強制父層旋轉與已經 ResetXForm 的 Mesh 同步
                        originalParent.rotation = node.rotation
